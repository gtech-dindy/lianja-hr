...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			setEditing: function (state) { //set edit state
				//console.log("Trace: setEditing() state="+state);
				g.editing = state;
			},
			...
			addData: function (data, append) { //parse data
				...
				if (p.dataType == 'json') {
					...
					{
						//data = $.extend({rows: g.data.rows, page: p.page, total: p.total}, data);
						g.data.rows.push(data.rows[0]);
						g.data.total += 1;
						data = g.data;
						//console.error("Trace: data.rows.length="+data.rows.length+", total="+data.total);
					}
				}
				g.data = data;
				if (p.preProcess) {
				...
				if (p.dataType == 'json') {
					...
					$.each(data.rows, function (i, row) {
						...
						//console.error("Trace: created new tr");
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								td.align = this.align;
								td.style.width = (p.colModel[idx].width+3) + "px";
								if (p.colModel[idx].type == 'G')
								{
									td.innerHTML = "<a onclick='Lianja.attachmentClicked(\"" + p.pageid + "\",\"" + p.sectionid + "\"," + row[ p.idProperty ] + "," + i + "," + idx + "," + "\"" + p.sectionid + "\"," + rowpos + ")'>Attachment</a>";
								}
								else if (p.colModel[idx].hyperlink)
								{
									td.innerHTML = "<a class='ui-lianja-hyperlink' onclick='Lianja.linkClickedGridColumnDeferred(\"" + p.colModel[idx].name + "\",\"" + p.onLinkClicked + "\",\"" + row[p.colModel[idx].name] + "\"" + ",\"" + p.pageid + "\",\"" + p.sectionid + "\"," + rowpos + ")'>" + row[p.colModel[idx].name] + "</a>";
								}
								else if (p.colModel[idx].type == 'T')
								{
									var text = row[p.colModel[idx].name].replace("T"," ");
								...
								if (p.colModel[idx].hidden)
								{
									$(td).css("display", "none");
								}
								last_td = td;
								td.id = p.uniqueid + 'row' + rowpos + '-col' + columnpos++;
								$(td).attr('abbr', $(this).attr('abbr'));
								$(tr).append(td);
								td = null;
							}
						);
						if (typeof last_td !== 'undefined')
						{
							last_td.style.width = "100%";
						}
						...
						$(tbody).append(tr);
						//console.error("Trace: added tr to tbody p.idProperty="+p.idProperty+", row[p.idProperty]="+row[p.idProperty]);
						if (typeof row[p.idProperty] === 'undefined')
						{
							row[ p.idProperty ] = rowpos+1;
						}
						if (row[p.idProperty] || row[p.idProperty]==0) {
							var id = p.uniqueid + 'row' + rowpos + '-rowid' + row[ p.idProperty ];
							tr.id = id;
							++rowpos;
						}
						tr = null;
				...
				if (!append) this.selectFirstRow();
				else this.selectLastRow();
			},
			rowCount: function()
			{
				return p.total;
			...
			changeSort: function (th) { //change sortorder
				...
				if (p.onChangeSort && p.sortable) {
					var axis = $(th).attr("axis");
					p.onChangeSort(parseInt(axis.substr(3)), p.sortorder);
				} else {
					this.populate();
				}
			},
			...
			populate: function () { //get latest data
				...
				////console.log("Trace: Lianja = "+typeof Lianja);
				if (typeof Lianja !== "undefined")
				{
					Lianja.OData(
						{
						type: 'readsync',
						...
						success: function (data) {
							g.primarykey = data.__primarykey;
							g.tabletype = data.__tabletype;
							g.addData(data);
							g.rawdata = data;
							if (p.calleronsuccess) p.calleronsuccess(p.callerargs);
						},
						error: function (XMLHttpRequest, textStatus, errorThrown) {
							g.rawdata = [];
							try {
								if (p.onError) p.onError(XMLHttpRequest, textStatus, errorThrown);
								if (p.calleronerror) p.calleronerror(p.callerargs);
							} catch (e) {}
						}
					});
				}
			...
			selectFirstRow: function() {
				g.editing = false;
				var tr = $('tbody tr:first', g.bDiv);
				if (typeof tr.id === 'undefined' || tr.id === '') tr = tr[0];
				$(tr).click();
				//$('tbody tr:first', g.bDiv).click();
				if (p.onClick) {
					p.onClick(tr, g, p);
				}
			},
			selectLastRow: function() {
				g.editing = false;
				$('tbody tr:last', g.bDiv).click();
				$(p.divid).parent().animate({
					scrollTop: $(p.divid).height()
				}, 0);
				if (p.onClick) {
					var tr = $('tbody tr:last', g.bDiv);
					if (typeof tr.id === 'undefined') tr = tr[0];
					p.onClick(tr, g, p);
				}
			},
			setSelectedRow: function(row) {
				g.editing = false;
				$("tbody tr:nth-child(" + row + ")", g.bDiv).click();
				if (p.onClick) {
					p.onClick($("tbody tr:nth-child(" + row + ")", g.bDiv), g, p);
				}
			},
			addRowProp: function () {
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					//console.log("Trace: selected row click");
					if (p.onClick) {
						p.onClick(this, g, p);
					}
					...
					if (g.multisel && ! (e.ctrlKey || e.metaKey)) {
						...
						//console.log("Trace: selected row mouseup");
						if (p.onClick) {
							p.onClick(this, g, p);
						}
					}
		...
		if (!p.buttons) $(g.bDiv).css("top", "25px !important");
		//if (p.mobile || ) $(g.bDiv).css("bottom", "0px !important");
		
		$('thead tr:first th', g.hDiv).each(function () {
			var thdiv = document.createElement('div');
			if ($(this).attr('abbr') || p.sortable) {
		...
		if (p.usepager) {
			...
			if (p.mobile)
			{
				html = ' <div class="pGroup"> <center><div class="pFirst pButton" title="First page"><span></span></div><div class="pPrev pButton" title="Previous page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <span> 1 </span>' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton" title="Next page"><span></span></div><div class="pLast pButton" title="Last page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton" title="Refresh page"><span></span></div> </div></span></center></div>';
			}
			else
			{
				html = ' <div class="pGroup"> <div class="pFirst pButton" title="First page"><span></span></div><div class="pPrev pButton" title="Previous page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input data-role="none" type="text" size="4" value="1" title="Current page"/> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton" title="Next page"><span></span></div><div class="pLast pButton" title="Last page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton" title="Refresh page"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
			}
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select data-role='none' name='rp'>" + opt + "</select></div> <div class='btnseparator'></div>");
	...
	$.fn.flexAddData = function (data, append) { // function to add data to grid
		return this.each(function () {
			if (this.grid) this.grid.addData(data, append);
		});
	};
	$.fn.flexEditing = function (state) { // function to set editing flag
		return this.each(function () {
			if (this.grid) this.grid.setEditing(state);
		});
	};
	...
	$.fn.flexGetPrimaryKey = function () { 
		var pk = undefined;
		this.each(function () {
			if (this.grid) pk = this.grid.getPrimaryKey();
		});
		return pk;
	};
	$.fn.flexGetTableType = function () { 
		var tt = undefined;
		this.each(function () {
			if (this.grid) tt = this.grid.getTableType();
		});
		return tt;
	};
	$.fn.flexSetSelectedRow = function (row) { // function to set current row
		return this.each(function () {
			if (this.grid) this.grid.setSelectedRow(row);
		});
	};
	$.fn.flexGrid = function () { // function to return internal grid
		return this.each(function () {
