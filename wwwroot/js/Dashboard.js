...
var ui = {
    ...
    tile_selector: '.tile',
    tile_color: 'bg-color-darken',
    tile_backcolor: "black",
    ...
    */
    attach: function (tile) {
		...
		// BM -- handle touchend to dispatch a click
		var obj = document.getElementById(tile.uniqueId);
		...
		// BM -- Chrome on Windows 8 does not properly fire the click event for some reason
        $(el).on("click", function(event) 
		{
            ...
            if (!_.isEmpty(tile.appUrl)) {
				...
				/* Lianja added this */
				if (tile.appUrl.indexOf("javascript:") == 0)
				{
					eval(tile.appUrl.substring(11));
					return;
				}
    ...
    */
    ...
    settings: function () {
        if (viewModel.user().isAnonymous)
		{
		 ...
		 {
			Lianja.navigate("cloudserver:/appcenter/login.rsp");
			return;
		 }
            ui.login();
		}
    ...
    reload: function () {
	      if (typeof(Lianja) !== "undefined")
		 {
			Lianja.navigate("cloudserver:/appcenter/appcenter.rsp");
			return;
		 }
...
$(document).ready(function () {
    ...
    $("#navbar").tooltip({
        title: "Come here to go back to Dashboard",
        animate: true,
        placement: 'bottom',
        trigger: 'manual'
    });
