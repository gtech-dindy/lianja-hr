	...
	scan
		...
		for lnRun = 1 to Alen(aNewNames)
			...
			If !Empty(m.cFirst)
				...
				case  Left(m.cFirst,1)="^")
					...
					lcLine = getAccessModifier(m.cProp, "Dimension");
					+ Substr(m.cFirst,2) + " = .f. " + Chr(10)
		...
		if m.nMethods > 0
			...
			for i=1 to alen(aMethods)
				...
				if !Empty(cFirst) and (at(cFirst, "function")=1 or at(cFirst, "procedure")=1)
	...
	for m.i=1 to alen(aProperties)
		...
		if nPos>0
			...
			otherwise
				...
				cPropStop = '"'
			endcase
			// fix up datetime and date values with and without "(" best here
