...
PDFJS.convertDataURIToBinary = function(dataURI) {
...
var DEFAULT_URL = undefined; //'compressed.tracemonkey-pldi-09.pdf';
var LOCAL_DATAURL = undefined;
...
PDFJS.setFile = function(file)
{
	DEFAULT_URL = file;
};

PDFJS.setDataUrl = function(data)
{
	LOCAL_DATAURL = data;
	DEFAULT_URL = undefined;
};

PDFJS.loadDataUrl = function(pdfAsDataUri)
{
	var pdfAsArray = PDFJS.convertDataURIToBinary(pdfAsDataUri);
	PDFViewerApplication.open(pdfAsArray, 0);
};

PDFJS.imageResourcesPath = './images/';
  PDFJS.workerSrc = 'file:///c:/lianja/library/pdfjs/build/pdf.worker.js';
...
function webViewerInitialized() {
  ...
  if (!window.File || !window.FileReader || !window.FileList || !window.Blob || PDFJS.disableopenfile) {
  ...
  if (file && file.lastIndexOf('file:', 0) === 0 && typeof LOCAL_URL === 'undefined') {
  ...
  if (file && typeof LOCAL_DATAURL === 'undefined') {
    PDFViewerApplication.open(file, 0);
  }
  else if (typeof LOCAL_DATAURL !== 'undefined')
  {
	  PDFJS.loadDataUrl(LOCAL_DATAURL);
  }
